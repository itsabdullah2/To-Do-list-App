...
let tasks = JSON.parse(localStorage.getItem("next-seven")) || [];
// Check to implement the tasks in local storage to page
if (localStorage.getItem("next-seven")) {
  tasks.map((task) => {
    createTask(task);
  });
}
...
document.addEventListener("input", (e) => {
  ...
  localStorage.setItem("next-seven", JSON.stringify(tasks));
});
...
function addTaskToArray() {
  ...
  if (input.value !== "") {
    ...
    localStorage.removeItem("next-seven");
  }
  // Clear Input
  input.value = "";
  input.focus();
